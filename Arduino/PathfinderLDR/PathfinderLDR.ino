...
IPAddress broadcastIP(10, 10, 10, 255); // broadcast ip to send UDP info over
unsigned int remotePort = 8000 + ID_NUM; // remote port to send UDP data to
...
void loop() {
  // read LDR value and send before driving
  int light_val = analogRead(ldr);
  char light_arr[4];
  itoa(light_val, light_arr, 10); //convert int to char array to send & avoid datatype confusion
  Serial.print("light: ");
  Serial.println(light_arr);
  Serial.println("writing to: " + String(broadcastIP) + " : " + String(remotePort));
  Udp.beginPacket(broadcastIP, remotePort);
  Udp.write(light_arr); // note that associated ID information with light value is encapsulated in port number 8000 + id
  Udp.endPacket();
