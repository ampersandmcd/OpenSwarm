function [Px, Py] = NumericWeightedLloydsAlgorithm(Px,Py, x_axis_sz, y_axis_sz, numIterations, showPlot, f_vals, sampling)
...
% modified version: takes f_vals as input for numerical integration instead
% of symbolic f
%
% f_vals must take form: [x, y, z_val]
% this function will then
% 1. normalize the z_vals between 0-1
% 2. compute, given sampling, necessary shifts for cartogram
% 3. map cartogram, find centroids & invert cartogram
close all
format compact
...
% 1. normalize input
zmin = min(f_vals(:, 3));
f_vals(:, 3) = f_vals(:, 3) - zmin; % ground bottom to zero
zmax = max(f_vals(:, 3));
f_vals(:, 3) = f_vals(:, 3) ./ zmax; % push top down to one
% now, f_vals(:,3) ranges from 0-1

% using a cartogram, map field to a uniform-density space
domain = unique(f_vals(:,1)); %domain must be x values for which we have numeric functional data
range = unique(f_vals(:,2)); %range must be y values for which we have numeric functional data
[xmesh, ymesh] = meshgrid(domain, range);
zmesh = reshape(f_vals(:,3), size(xmesh, 1), []); %reshape f_vals to create z_mesh
xfield = f_vals(:,1); %column of xvals
yfield = f_vals(:,2); %column of yvals
zfield = f_vals(:,3); %column of zvals
regularizer = 1;
...
for i = 1:size(cartofield_x)
    % numerically integrate wrt x and y using simpson's rule to get
    % density-based shifts
    xi = xfield(i);
    yi = yfield(i);
    
    % symbolic % xshift = (-1*integral(@(x) f(x, yfield(i)), 0, xfield(i)) + integral(@(x) f(x, yfield(i)), xfield(i), x_axis_sz)); 
    % get all fx values along this fixed y split between
    fx_lower = f_vals(f_vals(:,1) <= xi & f_vals(:,2)==yi, 3); %those less than this x on this yline and
    fx_upper = f_vals(f_vals(:,1) >= xi & f_vals(:,2)==yi, 3); %those greater than this x on this yline
    xshift = -1*simpsons(fx_lower, 0, xi, []) + simpsons(fx_upper, xi, x_axis_sz, []);
    
    % symbolic % yshift = (-1*integral(@(y) f(xfield(i), y), 0, yfield(i)) + integral(@(y) f(xfield(i), y), yfield(i), y_axis_sz));
    % get all fy values along this fixed y split between
    fy_lower = f_vals(f_vals(:,2) <= yi & f_vals(:,1)==xi, 3); %those less than this y on this xline and
    fy_upper = f_vals(f_vals(:,2) >= yi & f_vals(:,1)==xi, 3); %those greater than this y on this xline
    yshift = -1*simpsons(fy_lower, 0, yfield(i), []) + simpsons(fy_upper, yfield(i), y_axis_sz, []);
